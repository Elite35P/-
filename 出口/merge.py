...
# 2. Regex pattern: Match "进口量 (万吨), 同比%, 进口额 (亿元), 同比%, 均价 (美元/吨), 同比%" and "主要来自..." sections
pattern = re.compile(
    r"(大包粉|婴配粉|奶酪类|奶油类|乳清类|炼\s*乳|蛋白类|包装牛奶|酸奶类|稀奶油)"
    r".*?进口\s*([\d\.]+)\s*万吨.*?同比\s*([+\-]?[0-9\.]+)%"
    r".*?进口额\s*([\d\.]+)\s*亿元.*?同比\s*([+\-]?[0-9\.]+)%"
    r".*?均价\s*([\d\.]+)\s*美元\s*/\s*吨.*?同比\s*([+\-]?[0-9\.]+)%"
    r".*?主要来自([^。]+)。",
    re.S
)
...
for fname in files:
    ...
    for cat, vol, vol_yoy, val, val_yoy, price, price_yoy, sources in pattern.findall(text):
        records.append({
            '月份': month,
            '品类': cat.strip(),
            '进口量(万吨)': float(vol),
...
df.to_csv('品类进口明细汇总.csv', index=False)  # Save as CSV file